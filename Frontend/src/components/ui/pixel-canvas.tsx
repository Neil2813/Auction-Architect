...
/**
 * Single pixel entity that animates on the canvas
 */
class Pixel {
  width: number;
  height: number;
...
class PixelCanvasElement extends HTMLElement {
  ...
  private handleMouseEnterBound = () => this.handleAnimation("appear");
  private handleMouseLeaveBound = () => this.handleAnimation("disappear");
  private handleFocusBound = () => this.handleAnimation("appear");
  private handleBlurBound = () => this.handleAnimation("disappear");
  ...
  connectedCallback() {
    ...
    this._parent?.addEventListener("mouseenter", this.handleMouseEnterBound);
    this._parent?.addEventListener("mouseleave", this.handleMouseLeaveBound);

    if (!this.noFocus) {
      this._parent?.addEventListener("focus", this.handleFocusBound, {
        capture: true,
      });
      this._parent?.addEventListener("blur", this.handleBlurBound, {
        capture: true,
      });
    }
  }
  ...
  disconnectedCallback() {
    ...
    this._parent?.removeEventListener("mouseenter", this.handleMouseEnterBound);
    this._parent?.removeEventListener("mouseleave", this.handleMouseLeaveBound);

    if (!this.noFocus) {
      this._parent?.removeEventListener("focus", this.handleFocusBound, {
        capture: true,
      });
      this._parent?.removeEventListener("blur", this.handleBlurBound, {
        capture: true,
      });
    }
  ...
  private getDistanceToCenter(x: number, y: number) {
    const dx = x - this.canvas.width / 2;
    const dy = y - this.canvas.height / 2;
    return Math.sqrt(dx * dx + dy * dy);
  }

  private getDistanceToBottomLeft(x: number, y: number) {
    const dx = x;
...
/**
 * Let TS know about the <pixel-canvas> tag in JSX
 */
declare global {
  namespace JSX {
